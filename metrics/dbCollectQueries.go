...
func NewDbCollectQueries(logger logging.Logger, configuration *config.Config) *DbCollectQueries {
...
func (DbCollectQueries *DbCollectQueries) GetMetrics(metrics *Metrics) error {
...

	{
		...
		if err != nil {
			...
			if DbCollectQueries.configuration.CollectExplain {
				CollectionExplain(output_digest, "sum_time_us", DbCollectQueries.logger, DbCollectQueries.configuration)
				CollectionExplain(output_digest, "avg_time_us", DbCollectQueries.logger, DbCollectQueries.configuration)
			}
		}
...
func CollectionExplain(digests map[string]MetricGroupValue, filed_sorting string, logger logging.Logger, configuration *config.Config) {
	...
	for _, p := range pairs {
		...
		if digests[k]["schema_name"].(string) != "mysql" && digests[k]["schema_name"].(string) != "information_schema" && digests[k]["schema_name"].(string) != "performance_schema" && digests[k]["schema_name"].(string) != "NULL" && (strings.Contains(digests[k]["query_text"].(string), "SELECT") || strings.Contains(digests[k]["query_text"].(string), "select")) && digests[k]["explain"] == nil {
