...
function send_metrics() {
  ...
  curl -s -d "$JSON_STRING" -H "x-releem-api-key: $RELEEM_API_KEY" -H "Content-Type: application/json" -X POST https://api.hosting.releem.com/v1/mysql
}
...
function get_config() {
  ...
  if perl $MYSQLTUNER_FILENAME --json --verbose --notbstat --nocolstat --noidxstat --nopfstat --forcemem=$MYSQL_MEMORY_LIMIT --outputfile="$MYSQLTUNER_REPORT" --defaults-file ~/.my.cnf > /dev/null; then
      ...
      # Send metrics to Releem Platform. The answer is the configuration file for MySQL
      curl -s -d @$MYSQLTUNER_REPORT -H "x-releem-api-key: $RELEEM_API_KEY" -H "Content-Type: application/json" -X POST https://api.hosting.releem.com/v1/mysql -o "$MYSQLCONFIGURER_CONFIGFILE"
