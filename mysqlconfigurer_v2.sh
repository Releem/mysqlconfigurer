...
#  echo -n "Waiting for restarted mysql ${spin[0]}"
  ...
  while !(mysqladmin --user=${MYSQL_LOGIN} --password=${MYSQL_PASSWORD} ping > /dev/null 2>&1)
  do
    flag=$(($flag + 1))
    if [ $flag == 120 ]; then
#        echo "$flag break"
...
function releem_ps_mysql() {
    ...
    printf "\033[34m To apply changes to the mysql configuration, you need to restart the service\n\033[0m\n"
    FLAG_RESTART_SERVICE=1
    if [ -z "$RELEEM_RESTART_SERVICE" ]; then
        read -p "Please confirm restart MySQL service? (Y/N) " -n 1 -r
        echo    # move to a new line
    ...
    #echo "-------Test config-------"
    printf "\033[34m Restarting service with command '$RELEEM_MYSQL_RESTART_SERVICE'...\033[0m\n"
    eval "$RELEEM_MYSQL_RESTART_SERVICE" &
    wait_restart
    if [[ $(mysqladmin --host=127.0.0.1 --user=${MYSQL_LOGIN} --password=${MYSQL_PASSWORD} ping 2>/dev/null) == "mysqld is alive" ]];
    then
...
function get_config() {
  ...
  # Collect MySQL metrics
  if perl $MYSQLTUNER_FILENAME --json --verbose --notbstat --nocolstat --noidxstat --nopfstat --forcemem=$MYSQL_MEMORY_LIMIT --outputfile="$MYSQLTUNER_REPORT" --user=$MYSQL_LOGIN --pass=$MYSQL_PASSWORD --host=127.0.0.1 > /dev/null; then
