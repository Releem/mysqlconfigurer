...
import (
	...
	"task-automator/internal/db"
	"task-automator/internal/phase2"
)
...
func NewValidator(conn *db.Connection) *Validator {
...
func (v *Validator) checkStorageEngineForTable(tableName string, stmtResult *StatementValidationResult) error {
	...
	if len(parts) == 2 {
		...
		err := v.conn.DB.QueryRow("SELECT DATABASE()").Scan(&dbName)
		if err != nil {
			return err
		}
		tblName = tableName
...
func (v *Validator) checkTableSizeForTable(tableName string, stmtResult *StatementValidationResult) error {
	...
	// Get row count
	var rows int64
	err := v.conn.DB.QueryRow(fmt.Sprintf("SELECT COUNT(*) FROM `%s`.`%s`", dbName, tblName)).Scan(&rows)
	if err != nil {
...
func (v *Validator) validatePTOSCPrerequisites(tableName, ddl string, stmtResult *StatementValidationResult) error {
	...
	err2 := v.conn.DB.QueryRow("SHOW VARIABLES LIKE 'log_bin'").Scan(&logBinVar, &logBinValue)
	if err2 == nil {
		...
		if !logBinValue.Valid || logBinValue.String == "OFF" || logBinValue.String == "0" {
			...
			err3 := v.conn.DB.QueryRow("SHOW VARIABLES LIKE 'log_bin_basename'").Scan(&logBinBasenameVar, &logBinBasenameValue)
...
func (v *Validator) checkFlavorAndVersion(result *ValidationResult) error {
	var version string
	err := v.conn.DB.QueryRow("SELECT VERSION()").Scan(&version)
	if err != nil {
		return err
	}
	...
	// Determine flavor
	var flavor string
	err = v.conn.DB.QueryRow("SELECT @@version_comment").Scan(&flavor)
	if err != nil {
		return err
	}
...
func (v *Validator) checkBinaryLog(result *ValidationResult) error {
	var logBin string
	err := v.conn.DB.QueryRow("SELECT @@log_bin").Scan(&logBin)
	if err != nil {
		return err
	}
...
func (v *Validator) getStorageEngine(dbName, tableName string) (string, error) {
	...
	query := `
		...
		WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?
	`
	err := v.conn.DB.QueryRow(query, dbName, tableName).Scan(&engine)
	if err != nil {
		return "", err
	}
	return engine, nil
