...
func ProcessQueryExplainTask(task_details string, logger logging.Logger, configuration *config.Config, metrics *models.Metrics) (int, int, string) {
	...
	// schema, err := collectDbSchema(db, input.SchemaName, logger)
	if err != nil {
	...
	// Build result
	metrics.DB.Queries = []models.MetricGroupValue{{"schema_name": input.SchemaName, "query_id": Digest, "query": input.QueryText, "query_text": input.QueryText, "explain": explainResult, "avg_time_us": 0, "calls": 0, "sum_time_us": 0}}
